 ...
 *
 */
@Service(value = "fileSearchService")
public class FileSearchService {
	 ...
	 * @return
	 */
	public TranResult<Map> ls(JWTSubject jwt, Long siteId, String path, boolean hidden,String creator,String wjtype,String searchpath,int pageSize,Long currentPage) {
		...
		//StringString creator,String wjtype,String searchpath
		XObject query = new XObject().add("siteId", siteId).add("parentPath", filePath).add("owner",creator).add("wjtype",wjtype).add("searchpath",searchpath);
		if (!ROOT.equals(jwt.getLoginId())) {
			query.add("orgCode", orgCode);
		}
		logger.info("creator:"+creator+"==wjtype:"+wjtype+"==searchpath:"+searchpath);
		int rowCount = sqlSessionTemplate.selectOne("wcms.fileSearch.count", query);
		float pageSize2 = pageSize;
		int pageCount = (int) Math.ceil(rowCount/pageSize2);
		long min= 0l;
		min = (currentPage-1)*pageSize;
		long max= currentPage*pageSize;
		query.add("min",min).add("max",max);
		List<FsObj> files = sqlSessionTemplate.selectList("wcms.fileSearch.search", query);
